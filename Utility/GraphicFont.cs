...
public class GraphicFont
{
    ...
    [
        "common/font/fonticon_xinput.tex", "common/font/fonticon_ps3.tex", "common/font/fonticon_ps4.tex", "common/font/fonticon_ps5.tex",
    ...
    static GraphicFont()
    {
        ...
        if (gfd != null) FontIcons = new GraphicFont(gfd.Data);
    }
    ...
    {
        if (data.Length < 16) throw new Exception("Missing Header");

        using var mStream = new MemoryStream(data);
        using var reader  = new BinaryReader(mStream);

        Identifier = Encoding.ASCII.GetString(reader.ReadBytes(4));
        if (Identifier != "gftd") throw new Exception("Not a GFTD");

        Version = Encoding.ASCII.GetString(reader.ReadBytes(4));
        if (Version != "0100") throw new Exception($"GFTD Version {Version} not supported.");

        Icons = new GraphicFontIcon[reader.ReadInt32()];
        reader.ReadUInt32();

        if (data.Length != 16 * (Icons.Length + 1)) throw new Exception("incorrect data size");

        for (var i = 0; i < Icons.Length; i++) Icons[i] = new GraphicFontIcon(reader);
    }
}
