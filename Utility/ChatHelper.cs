...
public static class ChatHelper
{
    ...
    ///     If the signature for this function could not be found -or- The UiModule is
    ...
    public static unsafe void SendMessage(string message)
    {
        ...
        try
        {
            if (utf8->Length == 0) throw new ArgumentException("message is empty", nameof(message));

            if (utf8->Length > 500) throw new ArgumentException("message is longer than 500 bytes", nameof(message));
            ...
            if (utf8->Length != oldLength) throw new ArgumentException("message contained invalid characters", nameof(message));

            var uiModule = UIModule.Instance();
            if (uiModule == null) throw new InvalidOperationException("The UiModule is currently unavailable");
