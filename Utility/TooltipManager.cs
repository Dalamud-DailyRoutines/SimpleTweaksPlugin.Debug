...
public static unsafe class TooltipManager
{
    ...
    private static void TooltipDelegate(AtkEventType eventType, AtkUnitBase* atkUnitBase, AtkResNode* node)
    {
        ...
        if (eventType      == AtkEventType.MouseOver) AtkStage.Instance()->TooltipManager.ShowTooltip(atkUnitBase->Id, node, tooltip());
        else if (eventType == AtkEventType.MouseOut) AtkStage.Instance()->TooltipManager.HideTooltip(atkUnitBase->Id);
    }
