...
{
    [PluginService]
    private static IGameInteropProvider ImNotGonnaCallItThat { get; set; } = null!;
    ...
    public static AtkUnitBase* GetUnitBase(string name, int index = 1) =>
        (AtkUnitBase*)Service.GameGui.GetAddonByName(name, index).Address;
    ...
    public static T* GetUnitBase<T>(string? name = null, int index = 1) where T : unmanaged
    {
        if (string.IsNullOrEmpty(name))
        {
            ...
            if (attr != null) name = attr.AddonIdentifiers.FirstOrDefault();
        }
    ...
    public static AtkUnitBase* GetAddonByID(uint id)
    {
        ...
        for (var i = 0; i < UnitListCount; i++)
        {
            ...
            foreach (var j in Enumerable.Range(0, Math.Min(unitManager->Count, unitManager->Entries.Length)))
            {
                ...
                if (unitBase != null && unitBase->Id == id) return unitBase;
            }
        }
    ...
    public static AgentInterface* GetAgent(AgentId agentId) =>
        Framework.Instance()->GetUIModule()->GetAgentModule()->GetAgentByInternalId(agentId);
    ...
    public static HttpClient HttpClient
    {
        get
        {
            ...
            httpClient = new HttpClient
                (new SocketsHttpHandler { AutomaticDecompression = DecompressionMethods.All, ConnectCallback = happyEyeballsCallback.ConnectCallback });

            return httpClient;
        }
    }

    public static Extensions.PointerReadOnlySpanUnboxer<AtkResNode> GetNodeList
        (AtkUldManager* uldManager) => new ReadOnlySpan<Pointer<AtkResNode>>(uldManager->NodeList, uldManager->NodeListCount).Unbox();
