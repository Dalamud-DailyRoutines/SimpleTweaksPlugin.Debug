...
public static class ImGuiExt
{
    ...
    public static void SetColumnWidths(int start, float firstWidth, params float[] widths)
    {
        ...
        for (var i = 0; i < widths.Length; i++) ImGui.SetColumnWidth(start + i + 1, widths[i]);
    }
    ...
    private static void BuildUiColorLists()
    {
        ...
        foreach (var c in s)
        {
            if (uniqueSortedUiForeground.All(u => u.Dark != c.Dark)) uniqueSortedUiForeground.Add(c);

            if (uniqueSortedUiGlow.All(u => u.Light != c.Light)) uniqueSortedUiGlow.Add(c);
        }

        uniqueSortedUiForeground.Sort
        ((a, b) =>
            {
