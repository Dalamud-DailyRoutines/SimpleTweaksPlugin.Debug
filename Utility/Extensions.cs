...
public static class Extensions
{
    public static string GetDescription(this Enum @enum)
    {
        ...
        var attribs = member[0].GetCustomAttributes(typeof(DescriptionAttribute), false);
        return attribs.Length > 0 ? ((DescriptionAttribute)attribs[0]).Description : @enum.ToString();
    }
    ...
    public static SeString GetSeString(this Utf8String utf8String) =>
        Common.ReadSeString(utf8String);
    ...
    public static bool Cutscene
        (this ICondition condition) => condition[ConditionFlag.WatchingCutscene]   ||
    ...
    internal static IEnumerable<(FieldInfo Field, TAttribute? Attribute)> GetFieldsWithAttribute<TAttribute>
        (this object obj, BindingFlags flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic) where TAttribute : Attribute =>
        obj.GetType().GetFields(flags).Select(f => (f, f.GetCustomAttribute<TAttribute>())).Where(f => f.Item2 != null);
    ...
    public static unsafe T* GetPointer<T>(this StdVector<T> vector, int index) where T : unmanaged
    {
