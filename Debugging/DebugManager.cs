...
public partial class DebugConfig
{
    ...
    public List<string> Undocked = new();
}

public abstract class DebugHelper : IDisposable
{
    public STPDebug Plugin = null!;
...
public static class DebugManager
{
    ...
    private static          STPDebug  _plugin   = null!;
    ...
    {
        if (debugPages.ContainsKey(key)) debugPages[key] = action;
        else debugPages.Add(key, action);
    ...
    {
        if (debugPages.ContainsKey(key)) debugPages.Remove(key);
    ...
    public static void SetPlugin(STPDebug plugin) =>
        _plugin = plugin;
    ...
    public static void DrawDebugWindow()
    {
        ...
        if (sidebarSize < 150)
        {
            ...
            try
            {
                foreach (var k in debugPages.Keys)
                {
                    ...
                    if (s > sidebarSize) sidebarSize = s;
                }
            }
            catch (Exception ex)
            {
        ...
        if (ImGui.BeginChild("###debugPages", new Vector2(sidebarSize, -1) * ImGui.GetIO().FontGlobalScale, true))
        {
            ...
            ((s, s1) =>
                {
                    if (s.StartsWith("[") && !s1.StartsWith("[")) return 1;
            ...
            foreach (var k in keys)
            {
                ...
                {
                    if (!_plugin.PluginConfig.Debugging.Undocked.Remove(k)) _plugin.PluginConfig.Debugging.Undocked.Add(k);
        ...
        if (ImGui.BeginChild("###debugView", new Vector2(-1, -1), true, ImGuiWindowFlags.HorizontalScrollbar))
        {
            if (string.IsNullOrEmpty
                    (_plugin.PluginConfig.Debugging.SelectedPage) ||
                !debugPages.ContainsKey(_plugin.PluginConfig.Debugging.SelectedPage)) ImGui.Text("请选择调试页面。");
            else
            {
                try
                {
    ...
    public static void SeStringToText(SeString seStr)
    {
            ...
            else
            {
                if (type.IsArray && value is Array arr)
                {
                    if (ImGui.TreeNode($"Values##{member.Name}-{addr}-{string.Join("-", path)}"))
                    {
                        for (var i = 0; i < arr.Length; i++)
                        {
                            ...
                            (
                                addr,
                                new List<string>(path) { $"_arrValue_{i}" },
                                type.GetElementType() ?? throw new Exception("Invalid Element Type"),
                                arr.GetValue(i),
                                member
                            );
                        }
                ...
                else if (fixedBuffer != null)
                {
                    if (ImGui.TreeNode($"Fixed {ParseTypeName(fixedBuffer.ElementType)} Buffer##{member.Name}-{addr}-{string.Join("-", path)}"))
                    {
                        ...
                        {
                            display = ImGui.BeginChild
                            (
                                $"scrollBuffer##{member.Name}-{addr}-{string.Join("-", path)}",
                                new Vector2(ImGui.GetTextLineHeight() * 30, ImGui.GetTextLineHeight() * 8),
                                true
                            );
                            child = true;
                        }
    ...
    private static unsafe void PrintOutField
    (
        FieldInfo    f,
        LayoutKind   layoutKind,
        ref ulong    offsetAddress,
        ulong        addr,
        object       obj,
        List<string> path,
    ...
    )
    {
        ...
        if (ImGui.GetIO().KeyShift && ImGui.IsItemHovered()) ImGui.SetTooltip(fullFieldName);

        if (ImGui.GetIO().KeyShift && ImGui.IsItemClicked()) ImGui.SetClipboardText(fullFieldName);
            ...
            if (isOpen)
            {
                if (f.FieldType.GenericTypeArguments[0].IsPrimitive)
                {
                    ...
                    for (var i = 0U; i < fixedSizeArraySize; i++)
                    {
                        ...
                        using (ImRaii.PushColor
                                   (ImGuiCol.Text, ImGui.GetColorU32(ImGuiCol.TextDisabled)))
                ...
                else if (f.FieldType.GenericTypeArguments[0].FullName!.StartsWith("FFXIVClientStructs.Interop.Pointer`1"))
                {
                    for (var i = 0U; i < fixedSizeArraySize; i++)
                    {
                        ...
                        ImGui.SameLine();
                        using (ImRaii.PushColor(ImGuiCol.Text, ImGuiColors.DalamudOrange)) PrintAddress((void*)ptr);
        ...
        else
        {
            switch (fullFieldName)
            {
                case "FFXIVClientStructs.FFXIV.Client.Game.Object.GameObject.Name" when fixedBuffer != null:
                    ...
                    break;
                default:
                    if (fixedBuffer != null) PrintOutValue(addr + offsetAddress, new List<string>(path) { f.Name }, f.FieldType, f.GetValue(obj), f);
                    else
                    {
                        ...
                        else PrintOutValue(addr + offsetAddress, new List<string>(path) { f.Name }, f.FieldType, f.GetValue(obj), f);
                    }
    ...
    public static void PrintOutProperty(PropertyInfo p, LayoutKind layoutKind, ref ulong offsetAddress, ulong addr, object obj, List<string> path)
    {
        ...
        if (p.PropertyType.IsByRefLike || p.GetMethod == null || p.GetMethod.GetParameters().Length > 0) ImGui.TextDisabled("Unable to display");
        else PrintOutValue(addr, new List<string>(path) { p.Name }, p.PropertyType, p.GetValue(obj), p);
    }
    ...
    public static unsafe void PrintOutObject(object? obj, ulong addr, List<string> path, bool autoExpand = false, string? headerText = null)
    {
        ...
        if (obj is Utf8String utf8String)
        {
            ...
            try
            {
                ...
                if (s > 1) text = Encoding.UTF8.GetString(utf8String.StringPtr, s - 1);
            }
            catch (Exception ex)
            {
                err = ex;
            ...
            if (ImGui.TreeNode($"{headerText}##print-obj-{addr:X}-{string.Join("-", path)}"))
            {
                ...
                foreach (var f in obj.GetType().GetFields
                         (
                             ...
                             (ImGui.GetIO().KeyShift ? BindingFlags.NonPublic : BindingFlags.Public)
                         )) PrintOutField(f, layoutKind, ref offsetAddress, addr, obj, path);

                foreach (var p in obj.GetType().GetProperties()) PrintOutProperty(p, layoutKind, ref offsetAddress, addr, obj, path);
    ...
    public static unsafe void PrintAddress(void* address)
    {
        ...
        else ClickToCopyText(addressString);
    }
