...
public class ResetWindowPositions : DebugHelper
{
    ...
    public override void Draw()
    {
        ...
        ImGui.InputTextWithHint("###search", "Search...", ref searchInput);
        ...
        if (ImGui.BeginChild("buttonScroll", ImGui.GetContentRegionAvail()))
        {
            ...
            foreach (var l in windowList)
            {
                ...
                if (ImGui.Button
                        ($"##Button#{buttonIndex++}", new Vector2(ImGui.GetContentRegionAvail().X, ImGui.GetTextLineHeight() + ImGui.GetStyle().FramePadding.Y * 2)))
                {
                    resetWindowName                        =  l;
    ...
    private void DoReset()
    {
        ...
        if (ImGui.Begin
                (resetWindowName, ImGuiWindowFlags.NoSavedSettings | ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.NoBackground | ImGuiWindowFlags.NoInputs))
        {
            Service.PluginInterface.UiBuilder.Draw -= DoReset;
