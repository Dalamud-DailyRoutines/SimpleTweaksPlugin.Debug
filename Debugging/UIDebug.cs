...
public unsafe class UIDebug : DebugHelper
{
    ...
    private        bool         elementSelectorActive;
    private        int          elementSelectorIndex;
    private static float        elementSelectorCountdown;
    private static bool         elementSelectorScrolled;
    private static int          loadImageId;
    private static int          loadImageVersion;
    ...
    [
        "Depth Layer 1", "Depth Layer 2", "Depth Layer 3", "Depth Layer 4", "Depth Layer 5", "Depth Layer 6", "Depth Layer 7", "Depth Layer 8", "Depth Layer 9",
        "Depth Layer 10", "Depth Layer 11", "Depth Layer 12", "Depth Layer 13", "Loaded Units", "Focused Units", "Units 16", "Units 17", "Units 18"
    ];
    ...
    internal bool FindByAddress(AtkResNode* node, ulong address, out List<ulong>? path)
    {
        ...
        if (FindByAddress(node->PrevSiblingNode, address, out path) && path != null) return true;
    ...
    internal bool FindByAddress(AtkUnitBase* atkUnitBase, ulong address)
    {
        ...
        if (!FindByAddress(atkUnitBase->RootNode, address, out var path)) return false;
    ...
    public override void Draw()
    {
        ...
        if (ImGui.Button($"{(char)FontAwesomeIcon.ObjectUngroup}", new Vector2(-1, ImGui.GetItemRectSize().Y)))
        {
            ...
            if (elementSelectorActive) SetExclusiveDraw(DrawElementSelector);
        }
        ...
        foreach (var a in windows)
        {
            ...
            foreach (var n in a.Nodes)
            {
                ...
                if (nSelected && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
                {
                    ...
                    var l = new List<ulong> { (ulong)n.ResNode };
                ...
                (
                    n.State.Position       + ImGuiHelpers.MainViewport.Pos,
                    n.State.SecondPosition + ImGuiHelpers.MainViewport.Pos,
                    (uint)(nSelected ? 0x4400FFFF : 0x0000FF00)
                );
            }
    ...
    private List<NodeResult> GetAtkResNodeAtPosition(AtkUldManager* uldManager, Vector2 position, bool noReverse = false)
    {
        ...
        for (var i = 0; i < uldManager->NodeListCount; i++)
        {
            ...
            list.Add(new NodeResult { State = state, ResNode = node });
        }

        if (!noReverse) list.Reverse();
        return list;
    }
    ...
    public static void DrawUnitBase(AtkUnitBase* atkUnitBase)
    {
        ...
        {
            if (atkUnitBase->IsVisible) atkUnitBase->IsVisible = false;
            else atkUnitBase->Show(false, 0);
        }
                ...
                if (ImGui.BeginTable("atkUnitBase_atkValueTable", 3))
                {
                    ...
                    try
                    {
                        for (var i = 0; i < atkUnitBase->AtkValuesCount; i++)
                        {
                            ...
                            if (atkValue->Type == 0) ImGui.TextDisabled($"#{i}");
                            else ImGui.Text($"#{i}");

                            ImGui.TableNextColumn();
                            if (atkValue->Type == 0) ImGui.TextDisabled("Not Set");
                            else ImGui.Text($"{atkValue->Type}");
        ...
        else addonObj = *atkUnitBase;

        DebugManager.PrintOutObject(addonObj, (ulong)atkUnitBase, []);
        ...
        if (atkUnitBase->UldManager.NodeListCount > 0)
        {
            ...
            if (ImGui.TreeNode($"Node List##{(ulong)atkUnitBase:X}"))
            {
                ...
                for (var j = 0; j < atkUnitBase->UldManager.NodeListCount; j++) PrintNode(atkUnitBase->UldManager.NodeList[j], false, $"[{j}] ");
        ...
        if (atkUnitBase->CollisionNodeListCount > 0)
        {
            ...
            if (ImGui.TreeNode($"Collision List##{(ulong)atkUnitBase:X}"))
            {
                ...
                for (var j = 0; j < atkUnitBase->CollisionNodeListCount; j++) PrintNode(atkUnitBase->CollisionNodeList[j], false, $"[{j}] ");
        ...
        if (elementSelectorFind.Length > 0 && elementSelectorCountdown <= 0) elementSelectorFind = [];
    }
    ...
    public static void PrintNode(AtkResNode* node, bool printSiblings = true, string? treePrefix = "", bool textOnly = false)
    {
        ...
        if (elementSelectorFind.Length > 0 && elementSelectorFind[0] == (ulong)node)
        {
            ...
            (
                aPos - new Vector2(5),
                bPos + new Vector2(ImGui.GetWindowWidth(), 5),
                ImGui.ColorConvertFloat4ToU32(new Vector4(1, 1, 0, 0.5f * (elementSelectorCountdown / 100)))
            );
        }
    ...
    private static string NodeID(uint id, bool includeHashOnNoMatch = true)
    {
        ...
        if (!CustomNodes.TryGet(id, out var customNodeName)) customNodeName = customNodeIds.TryGetValue(id, out var nodeId) ? nodeId : string.Empty;
    ...
    private static void PrintSimpleNode(AtkResNode* node, string? treePrefix, bool textOnly = false)
    {
        ...
        if (elementSelectorFind.Length > 0) ImGui.SetNextItemOpen(elementSelectorFind.Contains((ulong)node), ImGuiCond.Always);
        ...
        if (ImGui.TreeNode($"{treePrefix} [{NodeID(node->NodeId)}] {node->Type} Node (ptr = {(long)node:X})###{(long)node}"))
        {
            ...
            {
                case NodeType.Text:
                    DebugManager.PrintOutObject(*(AtkTextNode*)node, (ulong)node, []);
                    break;
                case NodeType.Image:
                    DebugManager.PrintOutObject(*(AtkImageNode*)node, (ulong)node, []);
                    break;
                case NodeType.Collision:
                    DebugManager.PrintOutObject(*(AtkCollisionNode*)node, (ulong)node, []);
                    break;
                case NodeType.NineGrid:
                    DebugManager.PrintOutObject(*(AtkNineGridNode*)node, (ulong)node, []);
                    break;
            ...
            if (node->ChildNode != null) PrintNode(node->ChildNode);
            ...
            switch (node->Type)
            {
                case NodeType.Text:
                    ...
                    if (ImGui.TreeNode($"Payloads##{(ulong)textNode:X}"))
                    {
                        ...
                        for (var i = 0L; i < textNode->NodeText.BufUsed; i++) seStringBytes[i] = textNode->NodeText.StringPtr.Value[0];
                    ...
                    if (ImGui.InputText($"Replace Text##{(ulong)textNode:X}", ref s, 512, ImGuiInputTextFlags.EnterReturnsTrue)) textNode->SetText(s);

                    ImGui.Text
                    (
                        $"AlignmentType: {(AlignmentType)textNode->AlignmentFontType}[{textNode->AlignmentFontType:X2}]  FontSize: {textNode->FontSize}  CharSpacing: {textNode->CharSpacing}  LineSpacing: {textNode->LineSpacing}"
                    );
                    ...
                    ImGui.Text
                    (
                        $"BGColor: #{textNode->BackgroundColor.R:X2}{textNode->BackgroundColor.G:X2}{textNode->BackgroundColor.B:X2}{textNode->BackgroundColor.A:X2}"
                    );
                ...
                case NodeType.Counter:
                    ...
                    if (ImGui.InputText($"Replace Text##{(ulong)counterNode:X}", ref text, 100, ImGuiInputTextFlags.EnterReturnsTrue)) counterNode->SetText(text);

                    ImGui.InputInt($"##input_{(ulong)counterNode:X}", ref counterNodeInputNumber);
                    ImGui.SameLine();
                    if (ImGui.Button($"Set##{(ulong)counterNode:X}")) counterNode->SetNumber(counterNodeInputNumber);

                    break;
                case NodeType.NineGrid:
                    var ngNode = (AtkNineGridNode*)node;
                    ImGui.Text
                        ($"NineGrid Offsets:\tTop: {ngNode->TopOffset} Bottom: {ngNode->BottomOffset} Left: {ngNode->LeftOffset} Right: {ngNode->RightOffset}");
                ...
                case NodeType.Image:
                    ...
                    ImGui.SameLine();
                    if (ImGui.SmallButton("Load Icon")) iNode->LoadIconTexture((uint)loadImageId, loadImageVersion);
                    break;
                case NodeType.ClippingMask:
    ...
    private static void PrintTextureParts(AtkUldPartsList* partsList, uint partId)
    {
        if (partsList != null)
        {
            if (partId > partsList->PartCount) ImGui.Text($"part id({partId}) > part count({partsList->PartCount})?");
            else
            {
                    ...
                    if (ImGui.TreeNode($"Texture##{(ulong)kernelTexture->D3D11ShaderResourceView:X}"))
                    {
                        ...
                        */ // TODO: 7.3
                        if (ImGui.TreeNode($"Parts##{(ulong)kernelTexture->D3D11ShaderResourceView:X}"))
                        {
                            ImGui.BeginTable
                            (
                                $"partsTable##{(ulong)kernelTexture->D3D11ShaderResourceView:X}",
                                3,
                            ...
                            for (ushort i = 0; i < partsList->PartCount; i++)
                            {
                                ...
                                if (ImGui.Button
                                    (
                                        $"Switch##{(ulong)kernelTexture->D3D11ShaderResourceView:X}p{i}",
                                        new Vector2(-1, 23)
                                    )) partId = i;
                                ...
                                else ImGui.Text($"[U: {u}  V: {v}  W: {width}  H: {height}]");

                                if (tPart.UldAsset is not null                      &&
                                    tPart.UldAsset->AtkTexture.Resource is not null &&
                                    tPart.UldAsset->AtkTexture.Resource->KernelTextureObject is not null)
                                {
        ...
        else ImGui.Text("no texture loaded");
    ...
    private static void PrintComponentNode(AtkResNode* node, string? treePrefix, bool textOnly = false)
    {
        ...
        if (ImGui.TreeNode
            (
                $"{treePrefix} [{NodeID(node->NodeId)}] {objectInfo->ComponentType} Component Node (ptr = {(long)node:X}, component ptr = {(long)compNode->Component:X}) child count = {childCount}  ###{(long)node}"
            ))
        {
            if (ImGui.IsItemHovered()) DrawOutline(node);
            ...
            DebugManager.ClickToCopyText($"{(ulong)node:X}");
            ImGui.SameLine();
            DebugManager.PrintOutObject(*compNode, (ulong)compNode, []);
            ...
            switch (objectInfo->ComponentType)
            {
                case ComponentType.Button:
                    DebugManager.PrintOutObject(*(AtkComponentButton*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.Slider:
                    DebugManager.PrintOutObject(*(AtkComponentSlider*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.Window:
                    DebugManager.PrintOutObject(*(AtkComponentWindow*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.CheckBox:
                    DebugManager.PrintOutObject(*(AtkComponentCheckBox*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.GaugeBar:
                    DebugManager.PrintOutObject(*(AtkComponentGaugeBar*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.RadioButton:
                    DebugManager.PrintOutObject(*(AtkComponentRadioButton*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.TextInput:
                    DebugManager.PrintOutObject(*(AtkComponentTextInput*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.Icon:
                    DebugManager.PrintOutObject(*(AtkComponentIcon*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.NumericInput:
                    DebugManager.PrintOutObject(*(AtkComponentNumericInput*)compNode->Component, (ulong)compNode->Component, []);
                    break;
                case ComponentType.List:
                    DebugManager.PrintOutObject(*(AtkComponentList*)compNode->Component, (ulong)compNode->Component);
                    break;
                case ComponentType.TreeList:
                    DebugManager.PrintOutObject(*(AtkComponentTreeList*)compNode->Component, (ulong)compNode->Component);
                    break;
                case ComponentType.DropDownList:
                    DebugManager.PrintOutObject((AtkComponentDropDownList*)compNode->Component);
                    break;
                case ComponentType.ScrollBar:
                    DebugManager.PrintOutObject((AtkComponentScrollBar*)compNode->Component);
                    break;
                case ComponentType.ListItemRenderer:
                    DebugManager.PrintOutObject((AtkComponentListItemRenderer*)compNode->Component);
                    break;
                case ComponentType.IconText:
                    DebugManager.PrintOutObject((AtkComponentIconText*)compNode->Component);
                    break;
                case ComponentType.DragDrop:
                    DebugManager.PrintOutObject((AtkComponentDragDrop*)compNode->Component);
                    break;
                case ComponentType.GuildLeveCard:
                    DebugManager.PrintOutObject((AtkComponentGuildLeveCard*)compNode->Component);
                    break;
                case ComponentType.TextNineGrid:
                    DebugManager.PrintOutObject((AtkComponentTextNineGrid*)compNode->Component);
                    break;
                case ComponentType.JournalCanvas:
                    DebugManager.PrintOutObject((AtkComponentJournalCanvas*)compNode->Component);
                    break;
                case ComponentType.HoldButton:
                    DebugManager.PrintOutObject((AtkComponentHoldButton*)compNode->Component);
                    break;
#if DEBUG
                case ComponentType.Portrait:
                    DebugManager.PrintOutObject((AtkComponentPortrait*)compNode->Component);
                    break;
#endif
                default:
                    DebugManager.PrintOutObject(*compNode->Component, (ulong)compNode->Component, []);
                    break;
            }
            ...
            if (componentData != null)
            {
                ...
                switch (objectInfo->ComponentType)
                {
                    case ComponentType.Base:
                        DebugManager.PrintOutObject(componentData);
                        break;
                    case ComponentType.Button:
                        DebugManager.PrintOutObject((AtkUldComponentDataButton*)componentData);
                        break;
                    case ComponentType.Window:
                        DebugManager.PrintOutObject((AtkUldComponentDataWindow*)componentData);
                        break;
                    case ComponentType.CheckBox:
                        DebugManager.PrintOutObject((AtkUldComponentDataCheckBox*)componentData);
                        break;
                    case ComponentType.RadioButton:
                        DebugManager.PrintOutObject((AtkUldComponentDataRadioButton*)componentData);
                        break;
                    case ComponentType.GaugeBar:
                        DebugManager.PrintOutObject((AtkUldComponentDataGaugeBar*)componentData);
                        break;
                    case ComponentType.Slider:
                        DebugManager.PrintOutObject((AtkUldComponentDataSlider*)componentData);
                        break;
                    case ComponentType.TextInput:
                        DebugManager.PrintOutObject((AtkUldComponentDataTextInput*)componentData);
                        break;
                    case ComponentType.NumericInput:
                        DebugManager.PrintOutObject((AtkUldComponentDataNumericInput*)componentData);
                        break;
                    case ComponentType.List:
                        DebugManager.PrintOutObject((AtkUldComponentDataList*)componentData);
                        break;
                    case ComponentType.DropDownList:
                        DebugManager.PrintOutObject((AtkUldComponentDataDropDownList*)componentData);
                        break;
                    case ComponentType.Tab:
                        DebugManager.PrintOutObject((AtkUldComponentDataTab*)componentData);
                        break;
                    case ComponentType.TreeList:
                        DebugManager.PrintOutObject((AtkUldComponentDataTreeList*)componentData);
                        break;
                    case ComponentType.ScrollBar:
                        DebugManager.PrintOutObject((AtkUldComponentDataScrollBar*)componentData);
                        break;
                    case ComponentType.ListItemRenderer:
                        DebugManager.PrintOutObject((AtkUldComponentDataListItemRenderer*)componentData);
                        break;
                    case ComponentType.Icon:
                        DebugManager.PrintOutObject((AtkUldComponentDataIcon*)componentData);
                        break;
                    case ComponentType.IconText:
                        DebugManager.PrintOutObject((AtkUldComponentDataIconText*)componentData);
                        break;
                    case ComponentType.DragDrop:
                        DebugManager.PrintOutObject((AtkUldComponentDataDragDrop*)componentData);
                        break;
                    case ComponentType.GuildLeveCard:
                        DebugManager.PrintOutObject((AtkUldComponentDataGuildLeveCard*)componentData);
                        break;
                    case ComponentType.TextNineGrid:
                        DebugManager.PrintOutObject((AtkUldComponentDataTextNineGrid*)componentData);
                        break;
                    case ComponentType.JournalCanvas:
                        DebugManager.PrintOutObject((AtkUldComponentDataJournalCanvas*)componentData);
                        break;
                    case ComponentType.Multipurpose:
                        DebugManager.PrintOutObject((AtkUldComponentDataMultipurpose*)componentData);
                        break;
                    case ComponentType.Map:
                        DebugManager.PrintOutObject((AtkUldComponentDataMap*)componentData);
                        break;
                    case ComponentType.Preview:
                        DebugManager.PrintOutObject((AtkUldComponentDataPreview*)componentData);
                        break;
                    case ComponentType.HoldButton:
                        DebugManager.PrintOutObject((AtkUldComponentDataHoldButton*)componentData);
                        break;
#if DEBUG
                    case ComponentType.Portrait:
                        DebugManager.PrintOutObject((AtkUldComponentDataPortrait*)componentData);
                        break;
#endif
                    default:
                        DebugManager.PrintOutObject(componentData);
            ...
            switch (objectInfo->ComponentType)
            {
                ...
                case ComponentType.TreeList:
                    ...
                    if (ImGui.SmallButton("Inc.Selected")) l->SelectedItemIndex++;
            ...
            if (ImGui.TreeNode($"Node List##{(ulong)node:X}"))
            {
                ...
                for (var i = 0; i < compNode->Component->UldManager.NodeListCount; i++) PrintNode(compNode->Component->UldManager.NodeList[i], false, $"[{i}] ");
    ...
    private static void PrintResNode(AtkResNode* node)
    {
        ...
        ImGui.SameLine();
        if (ImGui.SmallButton($"T:Visible##{(ulong)node:X}")) node->NodeFlags ^= NodeFlags.Visible;

        ImGui.SameLine();
        if (ImGui.SmallButton($"C:Ptr##{(ulong)node:X}")) ImGui.SetClipboardText($"{(ulong)node:X}");
        ...
        if (ImGui.InputFloat2("Position", ref v2)) node->SetPositionFloat(v2.X, v2.Y);
    ...
    private static void DrawAtkTimeline(AtkResNode* node, AtkTimeline* timeline)
    {
        ...
        ImGui.Text($"Active Label Id: {timeline->ActiveLabelId} " + $"Duration: {timeline->LabelFrameIdxDuration} " + $"End Idx: {timeline->LabelEndFrameIdx}");
    ...
    private static void DrawAtkAnimation(string name, AtkResNode* node, AtkTimelineAnimation* anim)
    {
        ...
        {
            ImGui.Text($"Start: {anim->StartFrameIdx} " + $"End: {anim->EndFrameIdx}");
    ...
    private bool DrawUnitListHeader(int index, uint count, ulong ptr, bool highlight)
    {
        ...
        if (!string.IsNullOrEmpty(Plugin.PluginConfig.Debugging.AtkUnitBaseSearch) && !doingSearch) ImGui.SetNextItemOpen(true, ImGuiCond.Always);
        else if (doingSearch && string.IsNullOrEmpty(Plugin.PluginConfig.Debugging.AtkUnitBaseSearch)) ImGui.SetNextItemOpen(false, ImGuiCond.Always);
    ...
    private void DrawUnitBaseList()
    {
        ...
        for (var i = 0; i < UnitListCount; i++)
        {
            ...
            if (headerDrawn && headerOpen) ImGui.TreePop();

            if (!selectedInList[i] && selectedUnitBase != null)
            {
                foreach (var j in Enumerable.Range(0, Math.Min(unitManager->Count, unitManager->Entries.Length)))
                {
                    var unitBase = unitManager->Entries[j].Value;
        ...
        if (noResults) ImGui.TextDisabled("No Results");
        ...
        if (doingSearch       && string.IsNullOrEmpty(Plugin.PluginConfig.Debugging.AtkUnitBaseSearch)) doingSearch  = false;
        else if (!doingSearch && !string.IsNullOrEmpty(Plugin.PluginConfig.Debugging.AtkUnitBaseSearch)) doingSearch = true;
    }
