    ...
    public override void Draw()
    {
        ...
        if (ImGui.BeginTable("friends", 7, ImGuiTableFlags.Resizable))
        {
            ...
            for (var i = 0U; i < agent->InfoProxy->EntryCount; i++)
            {
                ...
                ImGui.TableNextColumn();
                if (!Service.Data.GetExcelSheet<ClassJob>().TryGetRow(friend->Job, out var job)) ImGui.TextDisabled("Unknown");
                else ImGui.Text($"{job.Abbreviation.ExtractText()}");

                ImGui.TableNextColumn();
                if (!Service.Data.GetExcelSheet<TerritoryType>().TryGetRow(friend->Location, out var location)) ImGui.TextDisabled($"Unknown");
                else ImGui.Text($"{location.PlaceName.Value.Name.ExtractText()}");
