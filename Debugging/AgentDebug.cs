...
public unsafe class AgentDebug : DebugHelper
{
    ...
    private static (Type agentType, AgentId[] agentIds)[] GetAgentTypes()
    {
        ...
        agentTypes.AddRange
        (
            typeof(AgentInterface).Assembly.GetTypes()
                                  .Select
                                  (t => (t, t.GetCustomAttributes(typeof(AgentAttribute))
                                             .Cast<AgentAttribute>()
                                             .Select(a => a.Id)
                                             .ToArray())
                                  )
        ...
        agentTypes.AddRange
        (
            typeof(STPDebug).Assembly.GetTypes()
                            .Select
                            (t => (t, t.GetCustomAttributes(typeof(AgentAttribute))
                                       .Cast<AgentAttribute>()
                                       .Select(a => a.Id)
    ...
    private void BuildSortedAgentList()
    {
        ...
        {
            l.Add(a);
            if ((uint)a > maxAgentId) maxAgentId = (uint)a;
        }
        ...
        if (ImGui.BeginTabBar("agenDebugTabs"))
        {
            if (ImGui.BeginTabItem("Agents"))
            {
                if (ImGui.BeginChild("AgentList", new Vector2(agentListWidth, -1), true))
                {
                    ...
                    if (ImGui.BeginChild("AgentListScroll", new Vector2(-1, -1)))
                    {
                        foreach (var agent in sortedAgentList)
                        {
                            ...
                            var size = ImGui.CalcTextSize($"{agent}")
                                       ...
                                       ImGui.GetStyle()
                                            .ScrollbarSize *
                                       2;
                            if (size > agentListWidth) agentListWidth = size;
                ...
                if (ImGui.BeginChild("AgentView", new Vector2(-1, -1), true))
                {
                    ...
                    if (selectedAgentTypes == null)
                    {
                        try
                        {
                            ...
                            selectedAgentType = selectedAgentTypes.FirstOrDefault
                                (t => t.FullName == selectedType, selectedAgentTypes.FirstOrDefault(typeof(AgentInterface)));
                        }
                        catch
                        {
                            selectedAgentTypes = [];
                    ...
                    if (agentInterface != null)
                    {
                        ...
                        if (mainModule != null)
                        {
                            ...
                            if (beginModule                                           > 0            &&
                                (ulong)agentInterface->AtkEventInterface.VirtualTable >= beginModule &&
                                (ulong)agentInterface->AtkEventInterface.VirtualTable <= endModule)
                            {
                                ImGui.SameLine();
                        ...
                        {
                            if (ImGui.Button("Disable Hook")) agentHook.Dispose();
                        }
                        ...
                        if (agentObj != null) DebugManager.PrintOutObject(agentObj, (ulong)agentInterface);
                        ...
                        else UIDebug.DrawUnitBase(addon);
                    }
                }
            ...
            if (ImGui.BeginTabItem("GetAgentByInternalID Calls"))
            {
                ...
                if (ImGui.BeginTable
                    (
                        "agentCallTable",
                        5,
                        ImGuiTableFlags.Borders | ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Sortable | ImGuiTableFlags.ScrollY,
                        ImGui.GetContentRegionAvail()
                    ))
                {
                    ...
                    foreach (var (agentId, getCounter) in agentGetLog)
                    {
                        ...
                        using (ImRaii.PushFont(UiBuilder.MonoFont))
                        {
                            ...
                            if (getCounter.AgentInterface != null) DebugManager.PrintAddress(getCounter.AgentInterface->VirtualTable);
                        }
                        ...
                        if (!getCounter.AgentStructType.IsValueCreated)
                        {
                            if (!a) a = getCounter.AgentStructType.Value != null;

                            ImGui.TextDisabled("Please Wait...");
                        }
                        else
                        {
                            if (getCounter.AgentStructType.Value.Length == 0) DebugManager.PrintOutObject(getCounter.AgentInterface);
                            else
                            {
                                foreach (var t in getCounter.AgentStructType.Value)
                                {
                                    ...
                                    if (agentObj != null) DebugManager.PrintOutObject(agentObj, (ulong)getCounter.AgentInterface);
                                }
                            }
                        }
                    }
    ...
    private AgentInterface* GetAgentByInternalIDDetour(void* agentModule, AgentId agentId)
    {
        ...
        try
        {
            ...
            if (agentGetLog.TryGetValue(agentId, out var counter)) counter.Count++;
            else agentGetLog.Add(agentId, new AgentGetCounter(agentId, ret));
        }
        catch (Exception ex)
        {
            SimpleLog.Error(ex);
    ...
    public override void Dispose()
    {
        ...
        foreach (var h in AgentEventHooks.Values) h.Dispose();
