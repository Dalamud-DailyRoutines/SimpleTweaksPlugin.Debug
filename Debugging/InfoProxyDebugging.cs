...
public unsafe class InfoProxyDebugging : DebugHelper
{
    ...
    public override void Draw()
    {
        if (infoProxies == null)
        {
            ...
            foreach (var a in typeof(InfoProxyInterface).Assembly.GetTypes().Select
                         (t => (t, t.GetCustomAttributes(typeof(InfoProxyAttribute)).Cast<InfoProxyAttribute>().ToArray())).Where(t => t.Item2.Length > 0))
        ...
        if (ImGui.BeginChild("proxy_select", new Vector2(250 * ImGui.GetIO().FontGlobalScale, 0), true))
        {
            for (var i = 0; i < 34; i++)
            {
                ...
                if (ImGui.Selectable($"{n}", SelectedProxy == p)) SelectedProxy = p;
            }
        }
        ...
        if (ImGui.BeginChild("proxy_view", Vector2.Zero, true))
        {
            ...
            if (infoProxies.TryGetValue(SelectedProxy, out var infoProxyInfo))
            {
                ...
                {
                    ImGui.SameLine();
                    DebugManager.PrintOutObject(proxyObj, (ulong)proxy, true);
                }
            ...
            {
                ImGui.SameLine();
                DebugManager.PrintOutObject(proxy, true);
            }
        }
