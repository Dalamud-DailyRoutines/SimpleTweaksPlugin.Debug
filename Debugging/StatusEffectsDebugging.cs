    ...
    private void DrawStatusExplorer(BattleChara* battleChara)
    {
        ...
        if (ImGui.BeginTable("statusTable", 7))
        {
            ...
            for (var i = 0; i < statusManager->Status.Length; i++)
            {
                ...
                if (s is { IsPermanent: true }) ImGui.Text("Permanent");
                else
                {
                ...
                if (status->SourceObject.ObjectId == 0xE0000000 || status->SourceObject.ObjectId == 0) ImGui.Text("None");
                else
                {
                    var sourceObj = CharacterManager.Instance()->LookupBattleCharaByEntityId(status->SourceObject.ObjectId);

                    if (sourceObj == null) DebugManager.ClickToCopyText($"0x{status->SourceObject.ObjectId:X}");
                    else
                    {
    ...
    public override void Draw()
    {
        if (ImGui.BeginTabBar($"statusExplorerTabs"))
        {
            if (ImGui.BeginTabItem($"Self"))
            {
                ...
                if (selfObject == null || selfObject->ObjectKind != ObjectKind.Pc) ImGui.Text("Self Not Found");
                else DrawStatusExplorer((BattleChara*)selfObject);
            ...
            {
                if (Service.Targets?.Target == null) ImGui.Text("No Target");
                else
                {
                    ...
                    else DrawStatusExplorer((BattleChara*)targetObject);
                }
            ...
            {
                if (Service.Targets?.FocusTarget == null) ImGui.Text("No Focus Target");
                else
                {
