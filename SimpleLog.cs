...
public sealed class SimpleLog
{
    ...
    {
        if (STPDebug.Plugin.PluginConfig.NoCallerInLog) return string.Empty;
...
#else
    public static void Verbose
        (object message, [CallerFilePath] string callerPath = "", [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = -1)
    {
    ...
    public static void Error
        (Exception ex, object message, [CallerFilePath] string callerPath = "", [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = -1)
    {
        foreach (var m in SplitMessage($"{message}\n{ex}")) PluginLog?.Error($"{m}");
    }

    public static void Error
        (Exception ex, string message, [CallerFilePath] string callerPath = "", [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = -1)
    {
        foreach (var m in SplitMessage($"{message}\n{ex}")) PluginLog!.Error($"{m}");
    }
#endif
    ...
    {
        if (message is IList list) return list.Cast<object>().Select((t, i) => $"{i}: {t}");
